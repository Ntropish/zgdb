...
// --- Type Definitions ---
type NewNodeData = { id: string; [key: string]: any };
const makeKey = (nodeType: string, id: string) => `${nodeType}/${id}`;
const textEncoder = new TextEncoder();

// --- Proxy for the Set-like object (e.g., zg.user) ---
export function createNodeSetProxy(context: GraphContext, nodeType: string) {
      ...
      get(target, prop, receiver) {
        ...
        if (prop === "get") {
          return (id: string) => {
            ...
            if (context.nodeCache.has(key)) return context.nodeCache.get(key);
  ...
  const proxy = new Proxy(
    {},
    {
      get(target, prop, receiver) {
        ...
        const relation = nodeConfig.relations[prop as string];
        if (relation) {
          const fbsObject = nodeConfig.deserialize(
            context.db.getSync(textEncoder.encode(cacheKey))!
          );
          ...
          if (relation.kind === "one-to-many") {
            ...
            for (const targetNode of targetIterator as Iterable<{
              id: string;
            }>) {
              const targetFbsObject = context.config.nodes[
                relation.target
              ].deserialize(
                context.db.getSync(
                  textEncoder.encode(makeKey(relation.target, targetNode.id))
                )!
              );
              if (
                targetFbsObject[relation.foreignKey!] &&
