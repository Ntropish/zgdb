...
export function generateClient(schema) {
    ...
    lines.push(`import { serializeNode, deserializeNode, supportedNodeTypes } from './generated-serializers.js';`);
    lines.push(`import { createNodeData, updateNodeData } from './mutation-helpers.js';`);
    ...
    lines.push("// ============================================");
    lines.push("/**");
    lines.push(" * Defines the contract for any storage backend. Implement this interface");
    lines.push(" * to connect the ZGDB client to your chosen database (e.g., Map, LevelDB, IndexedDB).");
    lines.push(" */");
    ...
    lines.push(`type NodeDataType<T extends ClientNodeType> = T extends 'user' ? UserData : T extends 'post' ? PostData : T extends 'tag' ? TagData : T extends 'familiar' ? FamiliarData : never;`);
    lines.push("");
    ...
    // --- createNode Method ---
    lines.push(`    /**`);
    lines.push(`     * Creates a new node, serializes it, and saves it to the store.`);
    lines.push(`     */`);
    lines.push(`    async createNode<T extends ClientNodeType>(`);
    lines.push(`      nodeType: T,`);
    lines.push(`      data: { fields: NodeDataType<T>['fields'], relationIds: NodeDataType<T>['relationIds'] }`);
    lines.push(`    ): Promise<NodeDataType<T>> {`);
    lines.push(`      const node = createNodeData[nodeType](data as any);`);
    lines.push(`      const key = KeyEncoder.nodeKey(nodeType, node.id).toString();`);
    ...
    lines.push(`     */`);
    lines.push(`    async getNode<T extends ClientNodeType>(nodeType: T, nodeId: string): Promise<NodeDataType<T> | undefined> {`);
    lines.push(`      const key = KeyEncoder.nodeKey(nodeType, nodeId).toString();`);
    lines.push(`      const buffer = await store.get(key);`);
    lines.push(`      if (!buffer) return undefined;`);
    lines.push(`      return deserializeNode[nodeType](buffer) as NodeDataType<T>;`);
    lines.push(`    },`);
    lines.push(``);
    // --- updateNode Method ---
    lines.push(`    /**`);
    ...
    lines.push(`        throw new Error(\`Node with key \${key} not found for update.\`);`);
    lines.push(`      }`);
    lines.push(``);
    lines.push(`      const existingNode = deserializeNode[nodeType](existingBuffer) as NodeDataType<T>;`);
    lines.push(`      const updatedNode = updateNodeData[nodeType](existingNode as any, recipe as any);`);
    lines.push(`      const updatedBuffer = serializeNode[nodeType](updatedNode as any);`);
    lines.push(``);
