...
async function generateTypesFile(
  ...
  const augmentedTypes = nodeNames
    .map((nodeName) => {
      ...
      const finalSchema = `export const ${capitalize(
        nodeName
      )}Schema = ${baseSchemaName}.extend({\n${relatedFields}\n});`;
      const finalType = `export type ${capitalize(
        nodeName
      )} = z.infer<typeof ${capitalize(nodeName)}Schema>;`;
...
`;
  ...
  const proxyInitializers = nodeNames
    .map(
      (name) =>
        `    client.${name} = new Proxy({} as Collection<${capitalize(
          name
        )}>, createApiProxyHandler(db, client, '${name}'));`
    )
    .join("\n");
