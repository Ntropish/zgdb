...
import { z } from "zod";
import { Schema, getEdgeTables } from "./utils.js";
...
export function generateSerializer(schema: Schema): string {
  ...
  for (const nodeType of nodeTypes) {
    ...
    for (const [fieldName, fieldType] of Object.entries(
      tableDef.fields.shape
    )) {
      if ((fieldType as any)._def.typeName === "ZodString") {
        lines.push(
          `        const ${fieldName} = builder.createString(data.fields.${fieldName});`
        );
      }
    }
    ...
    for (const [relationName] of Object.entries(tableDef.relations)) {
      ...
      lines.push(
        `          const relationIds = (data.relationIds.${relationName} as string[]).map(id => builder.createString(id));`
      );
      lines.push(
        `          ${relationName}Vector = ${capitalizedName}.create${capitalize(
          relationName
        )}IdsVector(builder, relationIds);`
      );
      lines.push(`        }`);
    }
    lines.push("");
    ...
    for (const [fieldName, fieldType] of Object.entries(
      ...
      if ((fieldType as any)._def.typeName === "ZodString") {
        lines.push(
          `        ${capitalizedName}.add${capitalize(
            fieldName
          )}(builder, ${fieldName});`
        );
      } else {
    ...
    lines.push(
      `        ${capitalizedName}.addUpdatedAt(builder, BigInt(data.updatedAt));`
    );
    ...
    lines.push(
      `        root = ${capitalizedName}.end${capitalizedName}(builder);`
    );
    lines.push(`        break;`);
    lines.push(`      }`);
  }
  ...
  for (const nodeType of nodeTypes) {
    ...
    for (const [relationName] of Object.entries(tableDef.relations)) {
      ...
      lines.push(
        `          relationIds.${relationName} = Array.from({ length: ${relationName}Length }, (_, i) => table.${relationName}Ids(i)!);`
      );
      lines.push(`        }`);
    }
    ...
    for (const [fieldName] of Object.entries(tableDef.fields.shape)) {
      const accessor =
        ...
        fieldName === "updatedAt"
          ...
          : `table.${fieldName}()!`;
      lines.push(
        `            ${fieldName}: ${accessor}${i < numFields - 1 ? "," : ""}`
      );
      i++;
    }
