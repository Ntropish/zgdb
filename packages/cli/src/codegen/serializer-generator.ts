...
function fbsFieldToTsMethod(s: string): string {
...
export function generateSerializer(schema: any): string {
  const lines: string[] = [];
  ...
  lines.push(
    ...nodeTypes.map(
      (t) =>
        `import { ${capitalize(t)} } from './graph-db/${toKebabCase(t)}.js';`
    )
  );
  ...
  for (const nodeType of nodeTypes) {
    ...
    for (const [relationName, relationDef] of Object.entries(
      nodeSchema.relations
    )) {
      const relationType = (relationDef as any)[0];
      lines.push(
        `    ${relationName}: ${
          relationType === "one" ? "string" : "string[]"
        };`
      );
    }
    lines.push(`  };`);
  ...
  for (const nodeType of nodeTypes) {
    ...
    for (const [fieldName, field] of Object.entries(nodeSchema.fields.shape)) {
      const zodType = (field as any)._def.typeName;
      if (zodType === "ZodString" || zodType === "ZodEnum") {
        offsetVariables.push(
          `    const ${fieldName}Offset = builder.createString(node.fields.${fieldName} || '');`
        );
      }
    }
    ...
    for (const [relationName, relationDef] of Object.entries(
      ...
      offsetVariables.push(
        `    const ${relationName}IdOffsets = (${ids}).map(id => builder.createString(id));`
      );
      offsetVariables.push(
        `    const ${tsMethodName}VectorOffset = ${capNodeType}.create${tsMethodStem}Vector(builder, ${relationName}IdOffsets);`
      );
    }
    ...
    lines.push(
      `    ${capNodeType}.addCreatedAt(builder, BigInt(node.createdAt));`
    );
    lines.push(
      `    ${capNodeType}.addUpdatedAt(builder, BigInt(node.updatedAt));`
    );
    ...
    for (const [fieldName, field] of Object.entries(nodeSchema.fields.shape)) {
      ...
      if (zodType === "ZodString" || zodType === "ZodEnum") {
        lines.push(
          `    ${capNodeType}.add${tsMethodStem}(builder, ${fieldName}Offset);`
        );
      } else if (zodType === "ZodNumber") {
        lines.push(
          `    ${capNodeType}.add${tsMethodStem}(builder, node.fields.${fieldName});`
        );
    ...
    for (const [relationName] of Object.entries(nodeSchema.relations)) {
      ...
      const tsMethodStem = capitalize(tsMethodName);
      lines.push(
        `    ${capNodeType}.add${tsMethodStem}(builder, ${tsMethodName}VectorOffset);`
      );
    }

    // --- Finish Building Table ---
    lines.push(
      `    const ${nodeType}Offset = ${capNodeType}.end${capNodeType}(builder);`
    );
  ...
  for (const nodeType of nodeTypes) {
    ...
    lines.push(
      `    const node = ${capNodeType}.getRootAs${capNodeType}(byteBuffer);`
    );
    ...
    for (const [relationName, relationDef] of Object.entries(
      ...
      lines.push(
        `    const ${relationName}Ids = Array.from({ length: node.${tsMethodName}Length() }, (_, i) => node.${tsMethodName}(i));`
      );
      if (relationType === "one") {
        lines.push(
          `    relationIds.${relationName} = ${relationName}Ids[0] || '';`
        );
      } else {
    ...
    lines.push(
      `      relationIds: relationIds as ${capNodeType}Data['relationIds'],`
    );
    lines.push(`    };`);
    lines.push(`  },`);
  }
