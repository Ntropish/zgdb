...
function validateConfig(config) {
    if (!config?.schema?.nodes || !config?.schema?.edges) {
        throw new Error("Invalid configuration: must contain schema.nodes and schema.edges");
    }
}
function runFlatc(outputPath, fbsPath) {
    try {
        const flatcCommand = `flatc --ts -o "${outputPath}" "${fbsPath}"`;
        (0, child_process_1.execSync)(flatcCommand, { stdio: "inherit" });
    }
    catch (error) {
...
function generateFlatbuffersSchema(config) {
    ...
    for (const [name, nodeSchema] of Object.entries(config.schema.nodes)) {
        ...
        for (const [key, type] of Object.entries(nodeSchema.shape)) {
            const zodType = type._def.typeName;
            if (zodToFbs[zodType]) {
...
function generateInterfaceProperties(config, nodeName) {
    ...
    for (const edge of config.schema.edges) {
        if (edge.source === nodeName) {
            ...
            const propType = edge.cardinality === "one-to-many" ? `${targetType}[]` : targetType;
...
function generateRelationConfig(edge, nodeName) {
    if (edge.source === nodeName) {
...
function generateNodeConfig(config, nodeName) {
    ...
    const creatorCall = primitiveFields
        .map((field) => `${capName}Fbs.add${capitalize(field)}(builder, ${field}Offset);`)
        .join("\n    ");
    return `{
    // --- Serialization ---
...
function generateTypeScriptClient(config, outputPath) {
...
${nodeNames
        .map((name) => `import { ${capitalize(name)} as ${capitalize(name)}Fbs } from './schema_generated.js';`)
        .join("\n")}
...
export function createClient() {
    ...
    return zg as {
        ${nodeNames
        .map((name) => `${name}: { add: (data: any) => ${capitalize(name)}, get: (id: string) => ${capitalize(name)} | null } & Iterable<${capitalize(name)}>`)
        .join(",\n        ")}
    };
}
...
function generate(configPath, outputPath) {
    ...
    const fbsPath = path_1.default.join(outputPath, "schema.fbs");
    fs_1.default.writeFileSync(fbsPath, fbsSchema);
