...
async function main() {
    ...
    try {
        const newOrder = await db.transact(async (tx) => {
            ...
            for (const item of shoppingCart) {
                ...
                if (!product || product.fields.stock < item.quantity) {
                    throw new Error(`Not enough stock for product: ${product?.fields.name}`);
                }
                // 2. Decrement stock
        ...
        const finalProduct = await db.transact((tx) => tx.getNode("product", products[0].id));
        console.log(`Final stock for ${finalProduct?.fields.name}:`, finalProduct?.fields.stock);
    }
    catch (e) {
