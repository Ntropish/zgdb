...
function executeTrade(tx, portfolioId, stockId, type, shares) {
    ...
    if (type === "buy" && portfolio.fields.cashBalance < tradeValue) {
        console.log(`  - Trade Canceled: Insufficient funds to buy ${shares} of ${stock.fields.ticker}`);
    ...
    if (type === "sell" &&
        ...
        console.log(`  - Trade Canceled: Not enough shares to sell ${shares} of ${stock.fields.ticker}`);
...
function printPortfolioSummary(tx, portfolioId) {
    const portfolio = tx.getNode("portfolio", portfolioId);
    if (!portfolio)
        return;
    ...
    for (const holdingId of portfolio.relationIds.holdings) {
        ...
        if (holding && stock && holding.fields.shares > 0) {
            ...
            console.log(`  - ${stock.fields.ticker}: ${holding.fields.shares} shares @ $${stock.fields.currentPrice.toFixed(2)} = $${holdingValue.toFixed(2)}`);
        }
    }
    console.log(`-------------------------------------------`);
...
function main() {
    ...
    for (let day = 1; day <= 5; day++) {
        ...
        db.transactSync((tx) => {
            ...
            const tradeType = faker.helpers.arrayElement([
                "buy",
                "sell",
            ]);
