...
const User = new Map<string, Uint8Array<ArrayBufferLike>>();
const Post = new Map<string, Uint8Array<ArrayBufferLike>>();
const Tag = new Map<string, Uint8Array<ArrayBufferLike>>();
const Familiar = new Map<string, Uint8Array<ArrayBufferLike>>();
const PostTagEdge = new Map<string, Uint8Array<ArrayBufferLike>>();
...
User.set(
  ...
  serializeNode.user(
    updateNodeData.user(
      deserializeNode.user(User.get(janeDoe.id)!),
      (draft) => {
        console.log(
          "Updating user1 to establish the other side of the relation.",
          draft
        );
...
User.set(
  ...
  serializeNode.user(
    updateNodeData.user(
      deserializeNode.user(User.get(johnDoe.id)!),
      (draft) => {
