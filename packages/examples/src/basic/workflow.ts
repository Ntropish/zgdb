...
async function main() {
  ...
  const janeDoe = await db.transact(async (tx) =>
    tx.createNode("user", {
  ...
  const johnDoe = await db.transact(async (tx) =>
    tx.createNode("user", {
  ...
  const tag1 = await db.transact(async (tx) =>
    tx.createNode("tag", {
  ...
  const finalJohn = await db.transact(async (tx) =>
    tx.getNode("user", johnDoe.id)
  );

  console.log("Final User Data:", finalJohn);
  console.log(`User's post count: ${finalJohn?.relationIds.posts.length}`);

  const finalJane = await db.transact(async (tx) =>
    tx.getNode("user", janeDoe.id)
  );
