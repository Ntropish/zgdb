import { createClient, NodeDataTypeMap } from "./dist/graph/zgdb-client.js";
...
async function reserveStockAndBeginCheckout(db: DbClient, cartId: string) {
  return db.transact(async (tx) => {
    ...
    const cartItems: CartItem[] = JSON.parse(cart.fields.itemsJson);
...
async function processPayment(
  db: DbClient,
  cartId: string
): Promise<{ success: boolean; gatewayId?: string; finalTotal?: number }> {
  return db.transact(async (tx) => {
    const cart = await tx.getNode("cart", cartId);
    ...
    const discount = cart.relationIds.discount
      ...
      : null;
    const finalTotal =
      subTotal * (1 - (discount?.fields.percentage || 0) / 100);
...
async function finalizeOrder(
  ...
  return db.transact(async (tx) => {
    ...
    const shippingAddress = (await tx.getNode(
      "address",
      customer!.relationIds.addresses[0]!
    ))!;
    const cartItems: CartItem[] = JSON.parse(cart!.fields.itemsJson);

    // Create line items, shipments, and notifications
    const lineItems: NodeDataTypeMap["lineItem"][] = [];
    const shipments: Record<
      string,
      {
        warehouseId: string;
        address: string;
        lineItems: NodeDataTypeMap["lineItem"][];
      }
    > = {};
    for (const item of cartItems) {
      const product = await tx.getNode("product", item.productId);
    ...
    for (const doc of subDocs)
      await (tx.updateNode as any)(doc.type, doc.id, (d: any) => {
...
async function main() {
  ...
  const { customer, warehouse, cart, product1, product2 } = await db.transact(
    async (tx) => {
  ...
  try {
    ...
    const { success, gatewayId, finalTotal } = await processPayment(
      db,
      cart.id
    );
    ...
    const finalOrder = await finalizeOrder(
      db,
      cart.id,
      gatewayId!,
      finalTotal!
    ...
    const finalCustomer = await db.transact((tx) =>
      tx.getNode("customer", customer.id)
    );
    console.log(
      `\nFinal loyalty points for ${finalCustomer!.fields.name}: ${
        finalCustomer!.fields.loyaltyPoints
      }`
    );
  } catch (e: any) {
