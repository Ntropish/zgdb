...
function executeTrade(
  ...
  if (type === "buy" && portfolio.fields.cashBalance < tradeValue) {
    if (log)
      console.log(
        `      - TRADE FAILED: Insufficient funds to buy ${shares} of ${stock.fields.ticker}.`
      );
    return;
  }
  ...
  if (
    ...
    if (log)
      console.log(
        `      - TRADE FAILED: Not enough shares to sell ${shares} of ${stock.fields.ticker}.`
      );
    return;
  }
 ...
 */
function simulateMonth(tx: TxClient, stockIds: string[], currentMonth: number) {
  const monthNames = [
    "Jan",
    "Feb",
    "Mar",
  ...
  for (const stockId of stockIds) {
    tx.updateNode("stock", stockId, (s) => {
      ...
      if (Math.random() < STOCK_NEWS_CHANCE) {
        ...
        const newsImpact = ["Positive Earnings", "New Partnership"].includes(
          newsType
        )
        ...
        console.log(
          `    * NEWS: ${s.fields.ticker} - ${newsType}! Impact: ${(
            newsImpact * 100
          ).toFixed(2)}%`
        );
      }
      ...
      if (s.fields.currentPrice < 1) s.fields.currentPrice = 1; // Prevent stock from going to zero
    });
  }
}
...
function rebalancePortfolio(
  ...
  const stockMap = new Map(
    stockIds.map((id) => [id, tx.getNode("stock", id)!])
  );
  const sortedStocks = [...stockMap.values()].sort(
    (a, b) => a.fields.volatility - b.fields.volatility
  );

  const riskProfileTargets: Record<string, number[]> = {
  ...
  for (let i = 0; i < sortedStocks.length; i++) {
    ...
    const sharesToTrade = Math.floor(
      valueDifference / stock!.fields.currentPrice
    );
 ...
 */
function taxLossHarvesting(tx: TxClient, portfolioId: string) {
  const portfolio = tx.getNode("portfolio", portfolioId);
  if (!portfolio) return;
  console.log(
  ...
  for (const holdingId of portfolio.relationIds.holdings) {
    ...
    const unrealizedGain =
      (stock!.fields.currentPrice - holding!.fields.costBasis) *
    ...
    if (unrealizedGain < 0) {
      ...
      console.log(
        `      - Selling ${sharesToSell.toFixed(2)} shares of ${
          stock!.fields.ticker
        } to harvest loss of $${(-unrealizedGain).toFixed(2)}`
      );
  ...
  if (harvestedValue > 0) {
    ...
    if (safeStock) {
      ...
      const sharesToBuy = Math.floor(
        harvestedValue / safeStockNode.fields.currentPrice
      );
      console.log(
        `      - Reinvesting harvested cash into ${sharesToBuy} shares of ${safeStockNode.fields.ticker}.`
      );
      executeTrade(
        tx,
        portfolioId,
        safeStock.relationIds.stock,
        "buy",
        sharesToBuy,
        false
      );
...
// ============================================
function getPortfolioValue(tx: TxClient, portfolioId: string): number {
  const portfolio = tx.getNode("portfolio", portfolioId);
...
function printFinalReport(
  ...
  for (const portfolioId of portfolioIds) {
    ...
    const returnOnInvestment =
      (totalReturn / portfolio.fields.totalInvested) * 100;
    ...
    console.log(
      `  - Total Invested:      $${portfolio.fields.totalInvested.toFixed(2)}`
    );
    console.log(
      `  - Net Return:          $${totalReturn.toFixed(
        2
      )} (${returnOnInvestment.toFixed(2)}%)`
    );
    console.log(
      `  - Final Cash Balance:  $${portfolio.fields.cashBalance.toFixed(2)}`
    );
    ...
    for (const holding of holdings) {
      ...
      console.log(
        `    * ${stock.fields.ticker.padEnd(5)}: ${holding.fields.shares
          .toFixed(2)
  ...
  console.log(
    `NET LIFETIME RETURN:   $${totalReturn.toFixed(2)} (${totalROI.toFixed(
      2
    )}%)`
  );
  console.log("===========================================");
...
function main() {
  ...
  const { portfolioIds, stockIds } = db.transactSync((tx) => {
    ...
    const portfolioIds = portfolioConfigs
      .map((p) =>
        tx.createNode("portfolio", {
          fields: {
            name: p.name,
            cashBalance: p.initialCash,
            riskProfile: p.profile as
              | "Conservative"
              | "Moderate"
              | "Aggressive",
  ...
  db.transactSync((tx) =>
    printFinalReport(
      tx,
      portfolioIds,
      INITIAL_CASH + ANNUAL_INVESTMENT * SIMULATION_YEARS
    )
  );
}
