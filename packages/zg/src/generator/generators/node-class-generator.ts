...
function isSimpleRelationship(
  rel: any
): rel is Relationship & { type: undefined } {
  return rel && typeof rel.name === "string" && rel.type !== "polymorphic";
}
...
function preprocessRelationships(
  ...
  for (const rel of relationships) {
    ...
    if (rel.cardinality === "one") {
      processed.push({
        ...rel,
        foreignKey: rel.foreignKey ?? `${rel.name}Id`,
      });
    } else {
      // 'many' cardinality
      const remoteSchema = allSchemas.find((s) => s.name === rel.node);
      if (!remoteSchema)
        throw new Error(`Schema not found for relationship: ${rel.node}`);

      const remoteRel = (remoteSchema.relationships ?? []).find((r) => {
        return isSimpleRelationship(r) && r.name === rel.mappedBy;
      });
      ...
      if (remoteRel.type === "polymorphic") {
        // This case needs to be handled based on a clearer understanding of the polymorphic IR
        throw new Error(
          `Reverse side of a many-to-one cannot be polymorphic yet. Relationship: ${schema.name}.${rel.name}`
        );
      } else {
        const remoteSimpleRel = remoteRel as Relationship;
        remoteFk = remoteSimpleRel.foreignKey ?? `${remoteSimpleRel.name}Id`;
      }
      if (!remoteFk)
        throw new Error(
